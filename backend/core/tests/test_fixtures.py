...
def test_native_files_carry_ghana_national_grid(fixtures_dir):
    ...
    for path in (gen / "sample.gpkg", gen / "shp" / "parcels.shp"):
        ds = ogr.Open(str(path))
        ref = ds.GetLayer(0).GetSpatialRef()
        ref.AutoIdentifyEPSG()
        assert ref.GetAuthorityCode(None) == "2136", path
...
def test_seeded_community_overlap(fixtures_dir, fixture_manifest):
    ...
    width_ft = overlap.GetEnvelope()[1] - overlap.GetEnvelope()[0]
    # Axis-aligned in UTM, only approximately so in the national grid.
    assert width_ft == pytest.approx(seed["width_m"] * FT_PER_M, rel=0.02)
...
def test_seeded_building_in_flood_zone(fixtures_dir, fixture_manifest):
    ...
    hits = [b.GetField("property_id") for b in features(gpkg, "buildings")
...
def test_control_points_file_has_expected_columns(fixtures_dir):
    with open(fixtures_dir / "source" / "control_points.csv", newline="") as fh:
        ...
        assert reader.fieldnames == [
            "point_id", "description", "easting_ft_2136", "northing_ft_2136",
            "lon_wgs84", "lat_wgs84", "height_m", "source", "accuracy_m",
        ]
        rows = list(reader)
    if not rows:
